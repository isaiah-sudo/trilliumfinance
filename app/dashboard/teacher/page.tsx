...
import { UNITS_DATA } from '../lesson/unitsData';
...
export default function TeacherDashboard() {
  ...
  // Load classroom data when selectedClassId changes
  useEffect(() => {
    if (selectedClassId) {
  ...
  // Sync local settings state with context settings
  useEffect(() => {
    if (settings) {
  ...
  const currentClassroom = classrooms.find(c => c.id === selectedClassId) || {
    id: selectedClassId,
    className: dbClassName || 'Classroom Console',
    classCode: classCode || ''
  };
  ...
  if (role !== 'teacher') {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-6 text-slate-400">
        ...
        <p className="text-sm max-w-md">The teacher panel is only accessible to users with instructor role permissions assigned.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 pb-16">
      {/* Top Header Bar */}
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 bg-slate-900/60 p-6 rounded-3xl border border-slate-700/50 backdrop-blur-md shadow-2xl">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            {classrooms.length > 1 && (
              <span className="px-2 py-0.5 rounded-full bg-teal-500/10 text-[10px] font-black text-teal-400">
                {classrooms.length} Classrooms Active
              </span>
            )}
          </div>
          ...
          {currentClassroom.classCode && (
            <div className="flex items-center gap-3 pt-1">
              <span className="text-xs text-slate-400">Join Code for Students:</span>
        ...
        {/* Multi-Class Selector & Action Controls */}
        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
          ...
          {/* Preview Student View Switcher */}
          <button
            onClick={() => setTeacherPreviewMode(!teacherPreviewMode)}
            className={`flex items-center gap-2 px-4 py-2 rounded-2xl border text-xs font-bold uppercase tracking-wider transition-all cursor-pointer ${
              teacherPreviewMode 
          ...
          >
            ...
            {teacherPreviewMode ? 'Exit Simulation' : 'Student View'}
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2 border-b border-slate-800 pb-2">
        ...
        >
          <Users className="h-4 w-4" /> Student Roster & Stats ({roster.length})
        </button>
        ...
        >
          <Megaphone className="h-4 w-4" /> Broadcast ({announcements.length})
        </button>
        ...
        >
          <Settings className="h-4 w-4" /> Trading Rules
        </button>
      </div>

      {/* TAB 1: STUDENT ROSTER & DETAILED STATS */}
      {activeTab === 'roster' && (
        <div className="space-y-6">
          ...
          {rosterLoading ? (
            ...
            <div className="py-20 text-center bg-slate-900/40 border border-slate-800 rounded-3xl">
              ...
              <h3 className="text-white font-bold text-base">No Students Found</h3>
      ...
      {activeTab === 'lessons' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {/* Assigned Lessons List & Student Progress */}
          <div className="lg:col-span-2 space-y-4">
            ...
            {assignments.length === 0 ? (
              <div className="p-12 text-center bg-slate-900/40 border border-slate-800 rounded-3xl text-slate-400 text-xs italic">
                No lessons assigned yet. Pick a lesson from the form on the left to assign it!
              </div>
            ) : (
