...
export default async function NewsPage() {
  ...
  return (
    <div className="w-full space-y-6 relative">
      ...
      <div className="relative w-full overflow-hidden rounded-3xl bg-gradient-to-br from-[#1a2133]/90 via-[#131b2e]/95 to-[#101726]/90 border border-slate-700/50 p-6 md:p-8 shadow-2xl">
      ...
      {/* The 3 news cards combined: a tiny bit smaller in width than the Daily Market Briefing container, equal squares slightly taller than wide */}
      <div className="w-[96%] max-w-[96%] mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full items-stretch">
          {articles.map((article: NewsArticle, index: number) => (
            <div
              key={article.id}
              className="group relative w-full flex flex-col justify-between overflow-visible rounded-3xl bg-[#1a2133]/90 backdrop-blur-md border border-slate-700/50 hover:border-blue-500/40 hover:shadow-[0_0_30px_rgba(59,130,246,0.12)] transition-all duration-300 aspect-[1/1.05] z-10 hover:z-40 focus-within:z-40"
            >
              {/* Top Section: Photo Header taking 52% of card */}
              <div className="relative h-[52%] w-full overflow-hidden rounded-t-3xl bg-slate-900 shrink-0">
                <img
                  src={article.image}
                ...
                <div className="absolute top-4 left-4 z-20 flex h-8 w-8 items-center justify-center rounded-xl bg-slate-900/90 text-sm font-extrabold text-blue-400 border border-slate-700/50 shadow-md backdrop-blur-sm">
                  0{index + 1}
                </div>
              ...
              {/* Bottom Section: 48% of card - headline, summary & buttons with zero empty void */}
              <div className="flex-1 p-5 sm:p-6 flex flex-col justify-between overflow-visible">
                <div className="space-y-2.5">
                  ...
                  <div className="space-y-1.5">
                    <h3 className="text-base sm:text-lg font-black text-white leading-snug tracking-tight group-hover:text-blue-400 transition-colors duration-300 line-clamp-2">
                      {article.headline}
                    </h3>
                ...
                {/* Action Buttons */}
                <div className="pt-3 flex gap-2.5 shrink-0">
                  <Link
                  ...
                  <a
                    ...
                    className="px-3.5 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-300 hover:text-white text-xs font-bold rounded-xl transition-all flex items-center justify-center gap-1 active:scale-95"
